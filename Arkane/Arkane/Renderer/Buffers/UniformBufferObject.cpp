...
{
	m_usage = EBufferUsage::EBufferUsage_Dynamic;
	SetUnmapped();
}
...
bool UniformBufferObject::AllocBufferObject(const void* _data, uint32_t _allocSize, EBufferUsage _usage /*= EBufferUsage::EBufferUsage_Dynamic*/)
{
...
void UniformBufferObject::FreeBufferObject()
{
	if (IsMapped())
	{
		UnmapBuffer();
...
void UniformBufferObject::Reference(const UniformBufferObject& _other)
{
...
void UniformBufferObject::Reference(const UniformBufferObject& _other, uint32_t _refOffset, uint32_t _refSize)
{
	akAssertReturnVoid(!IsMapped(), "Buffer must be not mapped");
...
void UniformBufferObject::Update(const void* _data, uint32_t _size, uint32_t _offset /*= 0*/) const
{
